...
class AE(nn.Module):
    def __init__(self, args):
        ...
        self.decoder2 = nn.Sequential(
            nn.Linear(args.S_dim + args.NS_dim + args.C_dim, 2048),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(args.ae_drop),
            nn.Linear(2048, args.X_dim),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        self.decoder3 = nn.Sequential(
            nn.Linear(args.S_dim + args.C_dim, 2048),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(args.ae_drop),
            nn.Linear(2048, args.X_dim),
            nn.LeakyReLU(0.2, inplace=True),
...
class AttributeDec(nn.Module):
    def __init__(self, args):
        super(AttributeDec, self).__init__()
        self.args = args
        self.encoder = nn.Sequential(
            nn.Linear(args.X_dim, 2 * args.C_dim),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(2 * args.C_dim, args.C_dim)
        )
