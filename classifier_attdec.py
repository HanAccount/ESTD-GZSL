...
class CLASSIFIER:
    # train_Y is interger
    def __init__(self, netDec, data_ori, opt, _train_X, _train_Y, data_loader, test_seen_feature,test_unseen_feature, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20,
        ...
        if self.netDec:
            self.netDec.eval()
            self.input_dim = self.input_dim + opt.C_dim
            self.model = LINEAR_LOGSOFTMAX(self.input_dim, self.nclass)

            self.train_X = self.compute_dec_out(self.train_X, self.input_dim, self.data_ori[0])
            self.test_unseen_feature = self.compute_dec_out(self.test_unseen_feature, self.input_dim, self.data_ori[1])
            self.test_seen_feature = self.compute_dec_out(self.test_seen_feature, self.input_dim, self.data_ori[2])
    ...
    def compute_dec_out(self, test_X, new_size, ori_data):
        start = 0
        ntest = test_X.size()[0]
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                ...
                inputX_ori = Variable(ori_data[start:end].cuda(), volatile=True)
            else:
                inputX = Variable(test_X[start:end], volatile=True)
                inputX_ori = Variable(ori_data[start:end], volatile=True)
            # [1200, 128]
            # print(inputX.shape)
            feat1 = self.netDec(inputX_ori)
